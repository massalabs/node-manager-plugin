...
const MASSA_WALLET_URL = MASSA_STATION_URL + "plugin/massa-labs/massa-wallet/api/"
...
export async function isMassaWalletInstalled(): Promise<boolean> {
    ...
    return plugins.some((plugin: PluginInfo) => plugin.name === 'Massa Wallet' && plugin.status === 'Up');
}
